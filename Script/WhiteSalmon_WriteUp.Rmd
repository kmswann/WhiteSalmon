...
Below hydroelectric dams, the downstream discharge rates – measured in cubic feet per second (cfs) – become altered based on energy production (Grantham et al, 2014). These unnatural flows tend to lower the amount of water in the river during the dry season, as water is conserved in the upstream reservoir for continued energy production. This causes deleterious conditions for salmon habitat (URS, 2005). In 2011, the Condit Dam was removed from the White Salmon. It is assumed that dam removal will benefit salmon through not only having a physical barrier removed but through restoring natural stream flow conditions on the White Salmon River.
...
```{r echo=FALSE, fig.cap="White Salmon River Basin", out.width = '100%'}
knitr::include_graphics("C:/Users/krist/Box Sync/Spring 2020/R/Environmental_Data_Analytics_2020/WhiteSalmon/Plots/basinmap.png")
...
**WaterYear:** Based on a year of October - September
...
- Water Years 1941-2019: Period of record
...
- Basin delineation via StreamStats (streamstats.usgs.gov/ss/)
- Underwood gage latitude and longitude (https://waterdata.usgs.gov/wa/nwis/nwismap/?site_no=14123500&agency_cd=USGS)
- National Hydrography Dataset (NHD) (https://www.usgs.gov/core-science-systems/ngp/national-hydrography);  


**ArcGIS Image Service**  

- Elevation tinted hillshade basemap
- Condit Dam latitude and longitude (remote sensed via basemap).
...
knitr::kable(sum.stats, caption = "Descriptive statistics by period")

```


```{r fig3, echo=FALSE,message=FALSE,include=TRUE,fig.cap="Discharge during different periods on the White Salmon River."}
################################################################################################
#Box plot stuff
################################################################################################
...
```{r fig4, echo=FALSE,message=FALSE,include=TRUE,fig.cap="Monthly averages by period."}
#################################################################################################
...
knitr::kable(high.RI.table, caption = "Flood reutrn intervals modeled by period.")
...
```{r calculating the 7Q10, include =FALSE}
...
```{r fig6, echo=FALSE,message=FALSE,include=TRUE,fig.cap="Annual probability of exceeding the minimum Q7."}
...
```{r fig7, echo=FALSE,message=FALSE,include=TRUE,fig.cap="The number of low flow days in the White Salmon River based on the period of record and post dam removal period."}
#plot low flow days
low.days <- subset(por, Flow <= low.7Q10[1]); 
...
low.days.post <- subset(por, Flow <= low.7Q10.post[1]);
...
legend("topleft", legend=c("Days with flows less than period of record 7Q10", "Days with flows less than post dam removal 7Q10"), col = c("#e58601","#46acc8"), pch=c(19,19), bg="white", cex=0.8)
```
...
knitr::kable(low.RI.table, caption = "7Q10s on the White Salmon River. Note that the number of years are the years where flows occur at or below 7Q10 within the entire period of record based on the 7Q10 for the given period.")
```
...
```{r fig10, echo=FALSE,fig.width=7.25, fig.height=9, message=FALSE,include=TRUE,fig.cap=" Monthly LOESS curve fit to historic data (1941-2019 water years) on the White Salmon River. Vertical dashed line signifies Condit Dam removal."}
...
```{r fig11, fig.width = 7.25, fig.height=9, echo=FALSE,message=FALSE,include=TRUE,fig.cap="Monthly LOESS curve fit to 2004-2011 and 2012-2019 water years on the White Salmon River. Vertical dashed line signifies Condit Dam removal. "}
...
There is a significant difference between flow rates on the White Salmon for the given time periods reviewed here. Since the Condit Dam was removed, low flow rates are modeled via 7Q10 to have increased in value, peak flow rates are modeled via flood return interval to have decreased in value, and overall flow trends are not monotonic. The implications here are that there seems to be a trend of higher low flows and lower high flows, both of which will benefit salmon migration and the food web that salmon depend on. It is likely that low flow rates were kept artificially low due to dam management, especially in the early years of the dam. High flow rates similarly may have been a product of dam management, with peak flows being more pronounced due to peak electricity demand. The switch from monotonic to non-monotonic trends is likely a product of the freeing of the river. All of these conclusions however are limited by the years of data available since dam removal. More years of data are needed. 
...
Environmental Protection Agency, 2018, October. "Low Flow Statistics Tools: a how-to handbook for NPDES permit waivers". Accessed online: https://www.epa.gov/sites/production/files/2018-11/documents/low_flow_stats_tools_handbook.pdf 

Pesanti, Dameon. 2016, July 28. “White Salmon tributary lures young fish.” The Columbian. Accessed Online: https://www.columbian.com/news/2016/jul/28/white-salmon-tributary-lures-young-fish/

URS. 2005. Condit Dam Removal Draft SEPA Supplemental Environmental Impact Statement (SEIS). For Washington Department of Ecology. Accessed online. https://fortress.wa.gov/ecy/publications/documents/0506022.pdf 
